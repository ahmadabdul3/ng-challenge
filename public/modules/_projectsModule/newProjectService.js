...
function NewProjectService(DepartmentService, DeadlineService, ResourceService, ProjectService) {
	...
	function funcAddResourceId() {
		if(self.newProject.selectedResource && self.newProject.selectedResource !== '') {
			...
			self.newProject.resourceIdList.push({id: resourceId, name: self.newProject.selectedResource.name});
	...
	function funcAddNewProject() {
		...
		if(valid) {
			var newProject = ProjectService.addNoValidation(
				...
				function(value) { return function() { return DepartmentService.getById(value); } } ,
				function(value) { return function() { return DeadlineService.getById(value); } } ,
				function(value) { return function() { return ResourceService.getByObjectList(value); } } 
				);
			if(newProject.isSuccessful) {
				/*var newProjItem = newProject.newItem;
				newProjItem.department = function() { return DepartmentService.getById(newProjItem.refs.department.id); };
				newProjItem.deadline = function() { return DeadlineService.getById(newProjItem.refs.deadline.id); };
				newProjItem.resources = function() { return ResourceService.getByObjectList(newProjItem.refs.resources); };*/
	...
	function funcGetProjectResources(idList) {
		return ResourceService.getByObjectList(idList);
	}
